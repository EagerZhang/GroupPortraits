...
var zoom = (function(){
	...
	/* KEYBOARD */
		$(document).keydown(function(event) {
			...
			if (event.which === 38) {keyboardMap.down = true;event.preventDefault();}
			if (event.which === 40) {keyboardMap.up = true;event.preventDefault();}
		});
		$(document).keyup(function(event) {
			if (event.which === 38) {keyboardMap.down = false;event.preventDefault();}
			if (event.which === 40) {keyboardMap.up= false;event.preventDefault();}
		});
